...
# 生成配置文件
cd /opt/3proxy-web
/opt/3proxy-web/venv/bin/python3 /opt/3proxy-web/config_gen.py
...
python3 -m venv venv || {
    echo -e "\033[31m创建Python虚拟环境失败\033[0m"
    exit 1
}
...
def reload_3proxy():
    try:
        ...
        # 检查配置文件是否生成成功
        if not os.path.exists(PROXYCFG_PATH):
            return False
    ...
    ip = request.form['ip']
        ...
        try:
            ...
            if port < 1024 or port > 65535:
                return jsonify({'status': 'error', 'message': '端口必须在1024-65535之间'})
            
            # 检查端口是否已存在
            existing = db.execute('SELECT id FROM proxy WHERE port = ?', (port,)).fetchone()
            if existing:
                db.close()
                return jsonify({'status': 'error', 'message': f'端口 {port} 已被使用'})
...
cat > $WORKDIR/config_gen.py << 'EOFCFG'
import sqlite3
...
# --------- init_db.py（DB初始化） ---------
cat > $WORKDIR/init_db.py << 'EOFINIT'
import sqlite3
...
<body>
    ...
    <script>
        ...
        let proxyDetailModal = null;  // 保存模态框实例
        ...
        function viewProxyGroup(cSegment) {
            ...
            fetch(`/api/proxy_group/${cSegment}`)
                ...
                .then(proxies => {
                    ...
                    proxies.forEach((proxy, index) => {
                        ...
                        html += `
                            <tr class="proxy-row ${rowClass}">
                                ...
                                <td style="padding: 8px 4px;">
                                    <code style="color: #d63384; font-size: 0.9rem;">${escapeHtml(proxy.username)}</code>
                                </td>
                                <td style="padding: 8px 4px;">
                                    <div style="display: flex; align-items: center; gap: 4px;">
                                        <input type="text" class="form-control form-control-sm" 
                                               value="${escapeHtml(proxy.password)}" readonly 
                                        ...
                                        <button class="btn btn-sm btn-outline-secondary" 
                                                ...
                                                onclick="copyPassword('${escapeHtml(proxy.password).replace(/'/g, "\\'")}', ${proxy.id})"
                                                title="复制密码">
                    ...
                    // 如果已有模态框实例，先销毁
                    if (proxyDetailModal) {
                        proxyDetailModal.dispose();
                    }
                    
                    proxyDetailModal = new bootstrap.Modal(document.getElementById('proxyDetailModal'));
        ...
        function updateSelectedCount() {
            ...
            if (countElement) {
                countElement.textContent = selectedProxies.size;
            }
        }
