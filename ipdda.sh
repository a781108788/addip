...
import os, sqlite3, random, string, re, collections, ipaddress, socket
...
def export_selected():
    ...
    prefix = request.form.get('userprefix', '')
    if not csegs:
        flash("未选择C段")
        return redirect('/')
    db = get_db()
    ...
    mem.seek(0)
    # 文件名为前缀+所选C段
    filename = f"{prefix}_{'_'.join(csegs)}.txt"
...
def add_ip_config():
    ...
    iptype = 'range' if '{' in ip_input and '..' in ip_input else ('single' if ',' not in ip_input and '-' not in ip_input else 'list')
    # 存到DB
    db = get_db()
    db.execute('INSERT INTO ip_config (ip_str, type, iface, created) VALUES (?,?,?,datetime("now"))', (ip_input, iptype, iface))
    db.commit()
    db.close()
    # 临时添加
    if mode == 'temp':
        if '{' in ip_input and '..' in ip_input:
            ...
            if match:
                ...
                for i in range(s, e+1):
                    os.system(f"ip addr add {base}{i}/24 dev {iface}")
        else:
            # 单IP或多个
            for ip in re.split(r'[,\s]+', ip_input):
                if ip:
                    os.system(f"ip addr add {ip}/24 dev {iface}")
    # 永久添加
    elif mode == 'perm':
        with open(INTERFACES_FILE, 'a+') as f:
            f.write(f"\nup bash -c 'for ip in {ip_input};do ip addr add $ip/24 dev {iface}; done'\n")
            f.write(f"down bash -c 'for ip in {ip_input};do ip addr del $ip/24 dev {iface}; done'\n")
    flash("已添加IP配置")
    return redirect('/')
                ...
                <div class="col-12">
                    <div class="card shadow-sm p-4">
                        <div class="d-flex mb-2 align-items-center">
                            ...
                            <input id="exportUserPrefix" class="form-control form-control-sm ms-2" placeholder="导出文件用户前缀" style="width:120px">
        ...
        <div class="tab-pane fade" id="ip-pane" role="tabpanel" aria-labelledby="ip-tab">
            <div class="card shadow-sm p-4 mt-4">
                ...
                <form class="row g-3 align-items-center mb-3" method="post" action="/add_ip_config">
                    ...
                    <div class="col">
                        <label class="form-label">IP区间/单IP（如192.168.1.{4..254} 或 192.168.1.2,192.168.1.3）</label>
                        <input name="ip_input" class="form-control" placeholder="192.168.1.{4..254} 或 192.168.1.2,192.168.1.3" required>
                    </div>
                    <div class="col-auto">
...
function buildTable(data, filterVal="") {
    ...
    Object.keys(groups).sort().forEach((cseg,i)=>{
        ...
        // 获取本C段第一个的ip_range、port_range、user_prefix
...
document.getElementById('exportSelected').onclick = function(){
    ...
    let prefix = document.getElementById('exportUserPrefix').value;
    if(selected.length==0) { alert("请选择C段"); return; }
    let form = new FormData();
    selected.forEach(c=>form.append('csegs[]',c));
    form.append('userprefix', prefix || '');
    fetch('/export_selected', {method:'POST', body:form})
        .then(resp=>resp.blob())
        .then(blob=>{
            let a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            a.download = (prefix ? prefix : 'export')+'_'+selected.join('_')+'.txt';
            a.click();
        });
};
// 导出所选代理
